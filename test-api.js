...
async function testAPI() {
  ...
  const prompt = `请从以下可靠的新闻网站搜索${today}（今天）和${yesterday}（昨天）这两天内与科技相关的最新新闻，返回3条新闻标题，格式为JSON数组：
...
4. 每条标题不超过50字，但必须包含具体事件、数据或观点等实质内容
5. 确保内容多样性，每条新闻应该有明显不同的主题
6. 避免重复内容，即使表述不同也不要包含相同事件的新闻
7. 标题应该是完整的新闻标题，不要只是话题或概念
8. 返回格式：[{"title": "[${today}][36氪]新闻标题1"}, {"title": "[${yesterday}][新华网]新闻标题2"}]`;
